#!/usr/bin/env bash
# Install committed file evidence for CTO acceptance in the isolated M5 service.
set -euo pipefail
umask 077

OLD_COMMIT=e55166fcdd62513b7ddbf424a28c4885393f90b3
FIX_COMMIT=515f8c06fc0ef30ba95e877716b1015f4136f7c9
OLD_DIR=/opt/pds-bridge/candidates/v0.1-m5-exit-code-fix
...
as_source npm --prefix "$FIX_DIR" run typecheck >/tmp/pds-m5-review-check.log 2>&1 || fail 'typecheck failed; see /tmp/pds-m5-review-check.log'
(cd "$FIX_DIR" && as_source node --no-warnings --experimental-strip-types --test --test-concurrency=2 tests/*.test.ts) >>/tmp/pds-m5-review-check.log 2>&1 || fail 'tests failed; see /tmp/pds-m5-review-check.log'
...
previous=con.execute("SELECT state FROM tasks WHERE task_id='16fc0bf0-9a9e-4bc4-ab24-c090d8e04c6f'").fetchone()
con.close()
if active or previous != ('FAILED',):
    raise SystemExit('M5_FIX_FAILED: Tasks changed or pending; active=%d; initial=%s' % (active, previous))
PY
}
check_tasks
...
ROLLBACK=0
printf 'PDS_M5_REVIEW_UPDATE_BEGIN\nsourceCommit=%s\nM5=ready\nv003=active\noldTask=FAILED_PRESERVED\nbackup=%s\ntests=PASS\nPDS_M5_REVIEW_UPDATE_END\n' "$FIX_COMMIT" "$BACKUP"
